...
export const SURFACE_PATTERN_KINDS = ["wood", "marble", "strata"] as const;
export type SurfacePatternKind = (typeof SURFACE_PATTERN_KINDS)[number];
export const SURFACE_PATTERN_AXES = ["x", "y", "z"] as const;
export type SurfacePatternAxis = (typeof SURFACE_PATTERN_AXES)[number];

/** Sparse authored state. Absence of the whole object means no pattern. */
export interface SurfacePattern {
  kind: SurfacePatternKind;
  /** Cylinder/plane orientation. Hostile in-memory input falls back to y. */
  axis: SurfacePatternAxis;
  /** Periods across one normalized object-space unit. */
  scale?: number;
  /** Patterned-albedo blend. Absent resolves to 1. */
  strength?: number;
}

export type PatternKind = "none" | SurfacePatternKind;
export type PatternAxis = SurfacePatternAxis;
...
/** Compatibility name retained by the evidence harness. */
export type PatternParams = ResolvedSurfacePattern;

export const SURFACE_PATTERN_DEFAULT_AXIS: SurfacePatternAxis = "y";
export const SURFACE_PATTERN_SCALE_MIN = 0.5;
...
export const SURFACE_PATTERN_AXIS_WIRE_ID: Readonly<
  Record<PatternAxis, 0 | 1 | 2>
> = { x: 0, y: 1, z: 2 };
...
/**
 * Resolve authored state into the complete pattern domain. Total even for
 * malformed in-memory input: invalid/non-finite leaves fall back, numeric
 * values clamp only here, and absence resolves to kind none with strength 0.
 */
export function resolveSurfacePattern(
  ...
  const candidate = pattern as
    ...
    | undefined;
  const kind = SURFACE_PATTERN_KINDS.includes(
    candidate?.kind as SurfacePatternKind,
  )
    ? (candidate?.kind as SurfacePatternKind)
    : "none";
  if (kind === "none") {
  ...
  const axis = SURFACE_PATTERN_AXES.includes(
    candidate?.axis as SurfacePatternAxis,
  )
    ? (candidate?.axis as SurfacePatternAxis)
    : SURFACE_PATTERN_DEFAULT_AXIS;
  const scale = Number.isFinite(candidate?.scale)
    ? Math.max(
        SURFACE_PATTERN_SCALE_MIN,
        Math.min(SURFACE_PATTERN_SCALE_MAX, candidate?.scale as number),
      )
    : PATTERN_DEFAULT_SCALE[kind];
  const strength = Number.isFinite(candidate?.strength)
    ? Math.max(0, Math.min(1, candidate?.strength as number))
    : SURFACE_PATTERN_DEFAULT_STRENGTH;
  return { kind, axis, scale, strength };
}

export function surfacePatternKindFromWireId(id: number): PatternKind {
  if (id === 1) return "wood";
  if (id === 2) return "marble";
  if (id === 3) return "strata";
  return "none";
}

export function surfacePatternAxisFromWireId(id: number): PatternAxis {
  if (id === 0) return "x";
  if (id === 2) return "z";
  return "y";
}
...
/** Flowing, warped microveins used only after the footprint LOD opens. */
function marbleDetailRamp(axis: PatternAxis, scale: number, p: Vec3): number {
  ...
  // A second periodic family made V2 look like intersecting cracks at review
  // resolution. Keep one dominant flowing vein. A slow field admits only a
  // soft mineral branch in part of the stone, and caps it below the dark core.
  const branchGate = smoothstep(-0.04, 0.22, warp - 0.45 * warpB);
  const branch = 0.82 * branchGate * vein(branchPhase);
  const ramp = Math.max(cloud, vein(phase), branch);
  // The broad cloudy field needs enough fixed contrast to carry variance when
  // the macro/detail crossfade opens. This is a material constant, not a
  // fixture statistic; the lighter V3 palette keeps the emphasized vein from
  // returning to V2's nearly black corrosion network.
