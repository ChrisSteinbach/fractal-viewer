/**
 * An automated show's stop/advance conductor: the policy seam
 * between the show's OWN automation and everything else that wants to end
 * it. Born as the drift show's conductor; the timeline player's playback is
 * now conducted by a second instance of the same policy —
 ...
 * chokepoints that implement "any user edit stops the show" (the drift
 * show's stop-on-edit rule). Without a guard, every leg would stop the show it
 ...
 *   can never fire for a stop that didn't actually happen.
 ...
 *   an emptied-out or fully-corrupt collection). The dry stop is
 *   deferred until AFTER the flag unwinds: issued from inside the leg it
 *   would be swallowed by the own-leg guard — exactly the bug
...
export interface DriftPolicyDeps {
   ...
   * the show — no motion means no drift. */
   ...
   * toast for an implicit stop. Never called for a no-op'd
   * stop.
   */
...
export class DriftPolicy {
  ...
  /**
   * End the show — a STOP, not a pause (restarting is a fresh toggle).
   * A no-op while idle or while the show's own leg applies
   ...
   * preference sync, not the user reaching in elsewhere (the "Drift
   * stopped" toast is for those). Otherwise the leg runs under the own-leg
   * guard, so the replace-load chokepoints it flows through may call
   * {@link stop} freely without ending the show it belongs to; `launchLeg`
   * returns whether a leg actually launched — false means the source ran dry
   * (an emptied or fully-corrupt collection, an undecodable timeline step)
   * and ends the show AFTER the guard unwinds so the stop actually lands —
