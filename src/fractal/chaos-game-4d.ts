 ...
 *
 * The spike path is intentionally minimal: no variations, no final transform,
 * no symmetry (all of which the 3D path carries). Just affine maps and the
 * chaos game.
 */
 ...
 * {@link Transform4.weight}) and apply its composed affine, recording each
 * landing spot; the cloud converges on the system's 4D attractor.
 *
...
export function runChaosGame4(
  ...
  // amortisation `prepareChaosGame` does for the 3D path.
  ...
  for (let i = 0; i < WARMUP_ITERATIONS; i++) {
    const a = affines[pick()];
    const p = applyAffine4(a, x, y, z, w);
    x = p[0];
    y = p[1];
    z = p[2];
    w = p[3];
