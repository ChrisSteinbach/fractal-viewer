...
| **Surface tiling**                               | Scene / Look                   | Surface IFS/escape-time in 3D/4D, plus 3D Mandelbulb; finite groups match dimension; Points/Flame/Solid show the untiled attractor                                                         | Document                                                                                                                       | Keep one authored section visible across renderer inspection. **Tiling kind** chooses Reflection group or Mirrored lattice; show the finite **Reflection group** row or the lattice **Cell scale** row for the selected arm, with the shared analytic **Optional content clip** row separate from both. On/off, kind, group and clip edits re-enter an active Surface while preserving its view; cell scale is live; edits apply elsewhere on next Surface entry, and every tiling edit also re-renders the generated Flame backdrop (`trackAutoBackground`) even with Points' Auto-update off. Put finite dimension mismatch and the shared mesh-clip, Balloon, Symmetry and 4D-slab refusals adjacent to the editor. Keep imported non-picker clips as the authored sentinel rather than rewriting them, and leave the common on/off control available as the explicit clear path. |
