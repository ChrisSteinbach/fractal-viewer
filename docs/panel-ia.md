...
| **Finite Surface tiling**                        | Scene / Look                   | Surface IFS and escape-chain sessions in the group's matching 3D/4D dimension, plus the existing 3D-only Mandelbulb family; Points, Flame and Solid deliberately show the untiled attractor | Document                                                                                                                       | Keep the authored status visible across renderer inspection. The fixed reflection-group selector defines the chamber; an optional ShapeSpec clip only narrows its content and must remain a separate row. Edits re-enter an active Surface while preserving its view, or apply on next Surface entry elsewhere. Put dimension mismatch, mesh clip, Balloon, Symmetry and 4D slab refusals adjacent to the editor. Balloon/Symmetry make the dependent group/clip rows dormant while the common on/off control stays available to clear; wrong-dimension and mesh-clip selectors remain enabled as their recovery paths. Mesh-backed catalog choices remain authored but refused until the tiling clip owns a mesh-atlas binding. A preserved lattice block is disclosed but its pending renderer/control vocabulary is not exposed through these finite-only rows. |
