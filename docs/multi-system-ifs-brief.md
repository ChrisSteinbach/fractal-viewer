...
- `src/app/flame-file.ts` (~line 431) already detects the attribute and emits
  _"Xaos (per-transform chaos weights) is not supported and was ignored"_. Option A upgrades that warning
  into a feature and closes a real `.flame` interop gap.
- `src/fractal/chaos-game.ts` has exactly the right seam: a single `cumulative: Float64Array` weight
  table + `weighted` flag consumed by `pickIndex`, built once per run in the compose step, with
  kaleidoscope symmetry expanding transforms into slots that inherit their **base** map's weight.

### Design sketch

- `RunConfig` grows `chaosRows: Float64Array[] | null` — one cumulative row per **base** transform
  (χ is authored over base transforms, not kaleidoscope slots). `pickIndex(prevBase)` selects from
  `chaosRows[prevBase]` when present; the iterator threads `prevBase` (the base index of the last applied
  slot — the slot→base mapping already exists for coloring).
- Kaleidoscope slots inherit their base's row; a drawn base index is then expanded to a uniformly chosen
  slot of that base, preserving current symmetry semantics.
- Degenerate rows (all zero): fall back to the global weight table for that row and surface a warning —
  mirrors flam3's tolerance, avoids stuck orbits.
- Many-points note: with truly block-diagonal χ, one orbit never crosses blocks. The engine already runs
  batches of points; ensure initial transform choice is uniform over blocks (or reseed per sub-batch) so
  isolated blocks all get sampled. flam3's fuse (≈20 discarded iterations) matches ours.
- GPU flame path (`src/fractal/flame-gpu.ts`, `flame-gpu-4d.ts`): per-point state gains `prevBase`; rows
  upload as one row-major CDF texture/array; binary search or grain-table lookup per step, same as CPU.
- 4D: selection-layer only — `chaos-game-4d.ts` picks up the identical change. Dimension-agnostic.

### Import/export, UI, tests

- `flame-file.ts`: parse `chaos` rows (pad/truncate with 1s), export symmetric; remove the ignore-warning.
- UI: n×n matrix editor (grid of 0..2 sliders or numeric cells) behind an "Advanced" disclosure; two
  presets built from `presets.ts`'s existing `buildFern`/`mengerSponge`: **"Fern | Sponge (isolated)"**
  (block-diagonal) and **"1% leak"** (0.01 off-block).
- Tests: (a) transition-histogram test — empirical P(j|i) matches w_j·χ_ij within tolerance; (b) isolation
  invariant — zero cross-block visits at χ=block-diagonal; (c) `.flame` round-trip; (d) a golden file
  generated by Fractorium/flam3 with xaos, rendered and eyeballed once, then pixel-diffed.

**Effort: S–M. Recommended first — it unlocks the entire Part-1 feature space and fixes interop.**
...
### Point/flame/solid modes: the emitter transform
...
- New transform kind `emitter` with a **shape sampler**. First sampler: extruded 2D outline (closed
  polyline + height), sampled uniformly by area (or outline-only for a wireframe look). Ship a
  parametric **gear outline generator** (teeth N, outer/root radius, tooth width, axle hole) as the
  flagship shape; SVG-path and mesh samplers later.
- Emitter gets its own palette slot (cog color vs. structure color).
- All three point-consuming render modes (cloud, flame exposure, solid voxel) work with zero further
  changes.
- `.flame` interop: no faithful flam3 equivalent → route through the existing unsupported-feature warning
  path per `flame-interop.md` conventions. (Optional lossy export: a disc-shaped emitter ≈
  `gaussian_blur`; decide later.)
...
**Effort: S (emitter + gear sampler, all point modes), M–L (surface trap).**
...
### C′-color: trap as a palette channel (smallest diff in this brief)

The escape-time modes (`qjulia-de.ts`, `bulb-de.ts`, the `escape-de.ts` fold stacks incl. mandelbox)
already run the orbit at every shaded point. Add one accumulator inside that loop —
`trap = min(trap, sdShape(z_i))` — and expose `trap` as a new input to the existing color modes /
palettes. No change to marching or DE validity: it is coloring only. Evaluate it in a dedicated post-hit
orbit re-run at the hit point so the value is independent of marching step noise. Options: shape pose
(offset/rotate/scale in orbit space), min vs. first-threshold-crossing trap value, fade by iteration
index. 4D note: in quaternion mode the orbit lives in 4D — trap either a 3D shape applied to the sliced
coordinates or a genuine 4D shape; both are one-liners.

A mandelbox or quaternion-Julia surface shaded by distance-to-peace-sign reproduces the reference image
style, fully volumetric. `sdPeace3D(p) = min(sdTorus(p), three capsules)` — Tier-1 shapes below suffice.

Tests: mirror the accumulator in the CPU oracle and pixel-diff the GLSL, per the existing discipline.
...
1. **C′-color** — orbit-trap palette channel in the escape-time modes. (Smallest diff in the brief;
   lands the reference-image look in 3D immediately.)
2. **A** — xaos in the chaos game + `.flame` import/export + fern|sponge presets. (Interop fix + biggest
   conceptual unlock.)
3. **C-point** — emitter transform + gear sampler. (Immediate visual win across cloud/flame/solid.)
4. **B-point** — post-word stage with depth slider. (Tiny; "sponge of ferns" demo.)
5. **C-surface + C′-geometry** — shape traps as geometry in both estimator families, level-band option.
6. **B-surface** — level-dependent alphabet in the descent. (Stretch.)

Each step is independently shippable. Steps 1, 3, and 5 share the shape library — start it at Tier 1
(composed primitives) and let the tiers grow on demand.
