...
| **Renderer-owned Scene color**                   | Scene / Look / Renderer        | One visible section for the active renderer: Points color mode/4D color, Flame tone and palette, Solid's matching mode pattern plus Orbit palette, or Surface source/palette/speed                                                              | Document                                                                                                                       | Use the same **Scene color** name for the same current-view question, but never show another renderer's editor beside it. Points and Solid lead with the same current-dimensional selector and expose the same ramp/contrast and Axis Color dependents. Solid appends one **Orbit palette** choice, which reveals its Palette/Custom branch; the extra renderer capability does not add a second source hierarchy. Both presentations edit the one authored color definition, so Points and Solid cannot drift, but neither renderer exposes the other's section. Flame, Solid, and Surface retain their values while hidden. Surface Height/Radius may consume the stored ramp definition internally.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **Optics (per-transform Glass)**                 | Scene / Look                   | Surface descent IFS in 3D and native 4D (the closed-solid backend where the composition admits it; the estimator backend elsewhere — disclosed vacuous state on IFS geometry); refused for forward arms, the 3D fold core, software rasterizers | Document                                                                                                                       | Sits inside the shared Finish group (the third material sibling beside Finish and Pattern): the Glass bundle materializes the model alone, and the Distortion/Optical scale rows ship dormant until a model exists (the Pattern rows' family gate). Row edits re-enter Surface once at the editor's settlement boundary, the finish group's own timing; the group's restriction note names where the transport routes. The static note discloses the 4D coupling: scrubbing the slice or turning a w-plane rotor mid-session moves the displayed object off the composition the signed field describes, and the transmission degrades to honest refusals (unresolved dark), never an invented interior.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **Space tiling**                                 | Scene / Look                   | Points, Flame, Solid and Surface in both dimensions — Surface IFS/escape-time in 3D/4D plus 3D Mandelbulb; finite groups match dimension                                                                                                        | Document                                                                                                                       | Keep one authored section visible across renderer inspection. **Tiling kind** chooses Reflection group or Mirrored lattice; show the finite **Reflection group** row or the lattice **Cell scale** row for the selected arm, with the shared analytic **Optional content clip** row separate from both. Per-consumer edit timing is disclosed by the section, not copied per renderer: Points follows Auto-update/latest-wins; Flame restarts accumulation from the same seed; 3D Solid re-folds queries live; 4D Solid restarts the worker from the entry seed (images are baked into density); Surface re-enters preserving its view while lattice cell scale is live; and every tiling edit also re-renders the generated Flame backdrop (`trackAutoBackground`) even with Points' Auto-update off. Put finite dimension mismatch and the mesh-clip, lattice Balloon and 4D-slab refusals adjacent to the editor. Keep imported non-picker clips as the authored sentinel rather than rewriting them, and leave the common on/off control available as the explicit clear path. Finite Balloon composes; a lattice Balloon refusal keeps the kind selector usable to recover to finite reflections. |
| **Sphere inversion**                             | Scene / Look                   | Points (exact boundary sampler) and Surface `sphereInversion`/`sphereInversion4` sessions, 3D and native 4D; Flame and Solid refuse the family                                                                                                  | Document                                                                                                                       | After Transforms: the block is the scene's SUBJECT, replacing the transform system, so being drawn only by Points and Surface does not make it Renderer. One checkbox adds a showcase form in the current dimension and removes the block; Arrangement decides the dimension. Points follows Auto-update; a live Surface restarts without refit, selects at once and sliders on release. With no block, Flame/Solid disable the section beside the reason. Sliders span the measured public ranges; an out-of-span value widens its slider and reads as kept, and a resolver refusal is kept and disclosed beside its row, never clamped. With a block present (refused included), Transforms (except Auto-update), Xaos, Symmetry, Hybrid schedule, the final lens and finishes are DISABLED beside a reason naming the action; Background disables only its Flame choice. Removing the block re-enables them.                                                                                                                                                                                                                                                                                        |
...
| **Surface Depth of Field**                       | Renderer                       | Surface only, in every supported 3D and 4D session kind                                                                                                                                                                                         | Document                                                                                                                       | Keep it in a contextual Surface **Depth** section, separate from Points' **Depth Style**. It is off by default and live: toggling re-presents retained color/depth metadata without re-entering Surface or multiplying distance-estimator work.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Continuous zoom**                              | View / Device                  | Live camera in Points, Solid and Surface; Surface alone generates scale-adaptive analytic detail; Flame freezes camera input                                                                                                                    | Saved view                                                                                                                     | Keep it in the stable View section. Radius-to-lens handoff maintains the orbit target without crossing it; pan chooses that target. Surface raises contraction-aware depth on both backends up to a disclosed work cap. Points/Solid magnify their existing sample/grid and say so. A numeric FOV floor is disclosed; reset restores the ordinary lens and fits, while the toggle exits. Refuse a fresh start in Flame but keep an active toggle reachable so exit/reset cannot be stranded.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **Systems construction / Xaos editing**          | Workflow entry / Scene editing | Preset replacement and isolated-block construction enter the document; every renderer consumes the resulting system/Xaos support as implemented                                                                                                 | Loaded/copied result is document state                                                                                         | Put replace/add actions under Systems, and keep ONE replacement select: a complete authored scene document (the Surface Lit interiors) joins the preset menu as its own optgroup under a value prefix rather than as a second select beside it, with the differing timing disclosed in the section's shared hint. Composition source pickers that clone that menu skip the prefixed entries. Once created, put the Xaos matrix and composition state in Scene / Look, available across renderer inspection. Preserve apply-on-select for both replacement and block Add, and immediate persistent editing for Hybrid System B.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
