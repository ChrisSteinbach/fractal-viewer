...
**This is the canonical project-rules file for every agent.** Anything true
regardless of the harness lives here: invariants, refusals, frozen wire
constants, commands, and the one-line verdict of every settled question.
Harness-specific mechanics live in that harness's own file (`CLAUDE.md` imports
this one); nothing load-bearing may live there alone.

## What belongs here — rules here, evidence in `docs/` or its harness
...
question. **The EVIDENCE lives in `docs/` or the owning executable harness** —
measured numbers, measurement narratives, refuted hypotheses, the history of a
correction — linked from the rule or gate it supports.
...
The delayed escape-chain, ground-plane and balloon lifts established the
verdict: the algebra is usually shared; the later cost is structural — frozen
wire layout and independently re-derived definitions. The balloon Points lift
also fixed the dimensional-reduction-first rule: PROJECT THEN INVERT the point
actually displayed. The measurement history and why those decisions follow are
in `docs/4d-exploration.md`; the rules are below.
...
`node scripts/surface-4d-lift.verify.mjs --display=:0`. It drives eight
minimal `#v1=` scenes — no preset table, so the gate survives one changing —
and the three shipped 4D presets into Surface FROM THE UI. It asks what no
unit test reaches: does each session ENTER, reach a COMPLETED settle, DRAW,
and take the expected ENGINE, keeping `core:"escape4"` and the 4D
plane/balloon blocks live. DRAW is the non-backdrop share of a real screenshot;
a canvas READBACK outside a WebGL context's own rAF reads empty and must not be
used. The kaleidoscope fixture is deliberately LIGHT (2 maps at order 3): the
DE's superlinear order cost keeps a four-map order-5 system from settling with
or without the floor inside 200s on this hardware. Without `--display` the
engine column is reported rather than gated. Figures and run history:
`docs/4d-exploration.md`.
...
hash rather than the panel, which can disagree with the document. The figures
and caught refusal defect live in the script header. Not verified on WebKit or
Firefox Android.
...
clamp(vUv.y, 0, 1))` lines, the WGSL row form, its obliged-byte-exact
  ...
  - `surface-de.ts` — surface render's CPU oracle: `analyzeSurfaceSystem`
    ...
    gate. Pure-swirl FINALs (lens-only tag 4) use a global inverse certificate
    AND reciprocal full-hit-epsilon compensation in 3D/4D; enforce
    `SWIRL_LENS_MAX_RADIUS` and refuse nonlinear slabs. Evidence:
    `docs/swirl-surface-lens.md`, `scripts/surface-swirl.verify.mjs`.
...
- **`src/app/`** — Three.js + DOM glue. Vite root (`root: "src/app"`).
  ...
  - `surface-compute.ts` — WebGPU compute renderer; the ROUTING is the part
    ...
    rather than reaching the kernels, because WEBGPU REFUSES SILENTLY
    HERE. The live pane FITS (`fitSurfaceComputeRaster`); a capture TILES
    (`surfaceComputeTileRows`, capped at `SURFACE_COMPUTE_MAX_TILE_RAYS`).
    ...
    untouched WebGL path (`?surfacegl` forces it); compute-only routes exit
    with the disclosed toast instead. Full record in
...
6. **Verify** — all changes committed and pushed, and `git status` reports the
...
Use non-interactive flags for commands that can prompt: `cp -f`, `mv -f`,
`rm -f`, `ssh`/`scp -o BatchMode=yes`, and equivalent package-manager flags.
