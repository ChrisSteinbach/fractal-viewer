 ...
 * design, and the engine assertion is what makes the green row honest.
 * The scene routes are chosen to cover the pattern arm's frame
 * reconstruction:
 *
 * - lens3 (3D, fold-FINAL lens): the pattern source is the lens wrapper's
 *   winning-branch core query (patternFoldLensSource) — the multivalued
 *   fold case where no single matrix inverse exists.
 ...
 *   4D radius; the floor stays unpatterned inside the same frame. Its
 *   pattern authors ALL FOUR transforms: the xw rotation on transform 0
 *   puts map 0's copy outside the visible slice, so a single-slot pattern
 *   never fires there (attribution, not wire).
 *
 * THREE LEGS PER ROUTE, same-stage captured: unauthored (pattern absent),
 * patterned (a strong family on transform 0), and a strength-0 control
 * (family present on the wire, albedo mix at 0). The verdict pair:
 * patterned vs unauthored must differ STRUCTURALLY above the floor (the
 * pattern reached pixels — a uniform never uploaded or a define a recompose
 * site dropped is invisible to every unit test), and strength-0 vs
 * unauthored must be an identity within the anti-floor (mix(base, full,
 * 0.0) == base exactly, so a family that fades to zero changes nothing).
 * The pattern-none control is deliberately NOT a second unauthored render:
 * it is a document with the SAME shader program compiled (pattern gate on)
 * and the same calibration — the strongest possible byte-level check that
 * strength 0 is an albedo identity on the real driver.
 *
 * SAME-STAGE CAPTURE is the finish.verify.mjs discipline, verbatim: both
 ...
 * capture parked.
 *
 * MEASURED (headless SwiftShader, 640x360, stage 1 unless noted, two
 * consecutive full runs agreeing): lens3 patterned (wood on transform 0)
 * vs none: 3.80% central-region structural, strength-0 vs none 0.000%;
 * ifs4plane patterned (marble on all four transforms): 1.06%, strength-0
 * 0.000%; escape3 (strata on the head transform) reached the SETTLED
 * 8-pass latch: 9.19%, strength-0 0.000%. All compared captures report
 * engine=webgl. The ifs4plane floor is 0.5% — its auto-framed 4D object
 * is a small part of the frame, so a 3D-scale floor would demand an
 ...
 * hold on a real hardware compiler as well as SwiftShader.
 *
 * Usage (build + `npm run preview` first — this measures a real build):
 *   node scripts/pattern.verify.mjs                  # sw, all scenes
 *   node scripts/pattern.verify.mjs --mode=x11::0    # real driver, headed
 *   node scripts/pattern.verify.mjs --scene=lens3 --stage=1
 *
 *   --url       app origin (default https://localhost:4173)
 *   --mode      sw (default) | x11:<display>
 *   --scene     lens3 (default) | escape3 | ifs4plane | all
 ...
 *               (default 0.005 — a family that fades to zero may leave a
 *               few hundred silhouette pixels of program-level scheduling
 *               noise, measured at 0.19% on the worst scene and 0.00% on
 *               the rest)
...
/** finish.verify.mjs's lens3 scenario hash, verbatim: LENS_BASE_HASH after
 * its boot auto-frame, re-encoded with the resulting camera pose. The 3D
 * fold-FINAL lens archetype — a 4-map Sierpinski base under a boxfold final
 * transform, the pattern arm's multivalued fold case. */
const LENS3_HASH =
...
/** surface-4d-lift.verify.mjs's ifs4plane document, verbatim: a 4-map 4D
 * IFS (sierpinski-class, with xw/yw rotations on two transforms) and the
 * floor — the plain 4D WebGL route, whose pattern frame lifts the hit's own
 * w before the inverse rotor. */
...
/** surface-4d-lift.verify.mjs's escape3plane document, verbatim: the
 * classic 3D Mandelbox (one -1.5-weight mandelbox link) with the floor —
 * the 3D ESCAPE route, whose pattern wire is the HEAD transform's
 * forward-orbit slot. */
const ESCAPE3_HASH =
 ...
 * strength-0 control reuses the same families with strength 0. The 4D scene
 * patterns ALL transforms: its transform 0 carries an xw rotation that puts
 * map 0's copy outside the visible slice, so a single-slot pattern never
 * fires there (the attribution is scene geometry, not the wire — the 3D
 * scenes pattern only transform 0 and light up ~4-5% of the frame). */
...
const SCENES = [
  ...
  },
  {
    ...
    // effect measures ~1.1%): a 3D-scale floor would demand an effect the
    // object cannot fill.
    floor: 0.005,
  },
  {
...
function parseArgs(argv) {
  ...
  if (!["lens3", "escape3", "ifs4plane", "all"].includes(args.scene)) {
    throw new Error(`--scene must be lens3, escape3, ifs4plane or all`);
  }
 ...
 * holds the completed mean. The byte-stability retry used by
 * finish.verify.mjs proved racy here — the strip's presents are async and
 * the settle re-arms mid-settle, so a byte-stable pair could catch a STALE
 * present, and legs of the same scene then disagreed run to run. The fixed
 * grace lands on the deterministic completed mean instead. */
...
// ---------------------------------------------------------------------------
// Diff (finish.verify.mjs's, verbatim).
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  const log = (line) => console.error(`[pattern] ${line}`);
  ...
  try {
    ...
    for (const scene of scenes) {
      ...
      // summary — every leg must have run the WEBGL tracer, because .5's
      // pattern math is GLSL-only and a compute leg renders unpatterned.
      for (const [r, cap] of [
        [un, capUn],
        [patterned, capPa],
        [strength0, capS0],
        ...
        if (cap.engine !== "webgl") {
          problems.push(
            `${r.leg} leg ran engine=${cap.engine ?? "none"} at the compared stage (${stageName(common)}), expected webgl`,
          );
        }
      }
...
main().catch((err) => {
  console.error(`[pattern] FATAL: ${err.stack ?? err}`);
  process.exit(2);
});
