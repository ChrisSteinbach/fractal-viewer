 ...
 * exhaustion/nonfinite DE, and counts those refusals per invocation. The
 * transport is surface-only: the participating medium this body once carried
 * was removed on its measured cost (see surface-lighting.ts).
 */
import {
...
export interface SurfaceLightingShaderDialect {
  language: "glsl" | "wgsl";
  /** One of the nine frozen vec4 lanes; expressions may index a light. */
...
export function surfaceLightingShaderSource(
  ...
  for (${mi} light = 0; light < 2; light++) {
    ...
    for (${mi} smp = 0; smp < ${SURFACE_LIGHTING_MAX_SURFACE_SAMPLES}; smp++) {
      ...
      ${f} weight = emitter.w * emitterCosine * cosine * visible /
        (3.141592653589793 * distance2 * ${lane(8)}.x);
  ...
  return def + hash + random + seed + visibility + disk + surface;
}
