...
| ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Finite/lattice presets, authoring, links, Points/Flame lifecycle and draw, disclosure and malformed decode   | `scripts/tiling-ui.verify.mjs`                                                                                | Surface/panel baseline plus scoped SwiftShader Points and Flame legs passed real Workers, copied links, latest-wins, negative frames, same-seed Off controls and 4D view-only restarts; the full `all` scope re-ran green on verified Iris 2026-09-02 with the production routing verdicts (B3/A4/F4/lattices → webgl/compute as authored) and every leg's structural negative control                                                                                                        |
| Finite-group matrix, lattice scale extrema, analytic clip and the Points crash-fallback, browser-driven      | `scripts/tiling-ui.verify.mjs --scope=matrix`                                                                 | 2026-09-02: all ten legs green on SwiftShader and verified Iris — six finite groups authored through the panel on identical bases (A3/B3/H3 on `octahedron`, A4/B4/F4 on `pentatope`) at 5.97–13.75% tiled/untiled difference, lattice extrema 1.25 (3D) 12.68% and 4.0 (4D) 11.06%, a3+gear-clip vs a3 2.21%; the cloud-worker crash fell back to the PERMANENT synchronous path which re-rendered the same tiled object with worker requests 4→4 and only the app's own fallback disclosure |
| Save-PNG readiness and tiled export classification for Points, Flame and Sampled Solid                       | `scripts/tiling-ui.verify.mjs --scope=export`                                                                 | 2026-09-02: all three legs green on both engines through the app's own export flow at 1x — downloads completed behind each renderer's own readiness wait, tiled/untiled exports differ 5.62% (Points) / 24.97% (Solid) / 20.01% (Flame) on Iris                                                                                                                                                                                                                                               |
| Flame GPU→CPU ladder's terminal arm with tiling                                                              | `scripts/tiling-ui.verify.mjs --scope=flame-cpu`                                                              | 2026-09-02: WebGPU disabled by launch flags on SwiftShader and on the real Iris display — the worker reported `backend: "cpu"` for the tiled 1M/1x accumulation, which completed and differed from its same-seed Off frame by 32.35%/31.92%                                                                                                                                                                                                                                                   |
| Minimal inverse affine/fold and forward, 3D/4D, WebGL/compute, clip, plane, progress and structural controls | `scripts/surface-lattice.verify.mjs` and `scripts/surface-tiling.verify.mjs`                                  | Both real-Iris route matrices passed (re-run 2026-09-02: surface-lattice 21/21, surface-tiling all rows including the 4D WebGL row the slower SwiftShader box of the day could not settle inside 180s)                                                                                                                                                                                                                                                                                        |
| Carrier choice, cross-engine fade, fog/DoF/radial background and completed Save PNG                          | `scripts/lattice-presentation.verify.mjs`                                                                     | 22/22 scene rows and ten capture comparisons passed (re-run 2026-09-02: 36 printed checks, exit 0)                                                                                                                                                                                                                                                                                                                                                                                            |
| Completed export, mid-drain cancellation, interaction and collection encoding with authored tiling           | `scripts/capture-export.verify.mjs --tiling=a3`                                                               | 15/15 (2026-08-31; re-run 2026-09-02 at a 480s download bound — the slower SwiftShader of the day measured the untiled first save at 209.8s and the tiled A3 export at 332.3s, past the old 240s budget, which aborted healthy drains; the assertion is unchanged)                                                                                                                                                                                                                            |
| Renderer teardown during submitted compute work                                                              | `scripts/surface-teardown.verify.mjs --lens --tiling=a3 --toggleId=__modeExit --toggles=20 --toggleGapMs=900` | 20/20 on real Firefox/Iris compute; exact A3 and hardware backend retained (re-run 2026-09-02)                                                                                                                                                                                                                                                                                                                                                                                                |
| Flame teardown while a TILED accumulation is parked on submitted work                                        | `scripts/flame-teardown.verify.mjs --tiling=a3 --toggles=12`                                                  | 2026-09-02 on real Firefox/Iris: exit 0, gpu=true, restarts 11/12 (the equality-guard no-op slot), backend never flipped to CPU, exact `{group:"a3"}` read back before and after the storm — the palette storm drives the tiled binding-8 plot adapter through every teardown                                                                                                                                                                                                                 |
| WebGPU-unavailable fallback and compute-only refusal with lattice authored                                   | `scripts/surface-fallback.verify.mjs https://localhost:5173 fg`                                               | 4D lattice painted through WebGL with visible disclosure; fold-4D lattice refused with WebGPU reason (re-run 2026-09-02)                                                                                                                                                                                                                                                                                                                                                                      |
| Phone target/layout and exact-number interaction                                                             | `scripts/panel-numeric-control.verify.mjs`                                                                    | 87 slider companions over 84 states passed (re-run 2026-09-02)                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Flame WebGPU kernels against their CPU oracles, tiling scenarios included                                    | `npm run bench:gpu` (CI: twelve scenario shards, the split that fits the 20-minute per-scenario stall cap)    | On this tree CI's twelve shards passed 2026-09-02 (PR and post-merge runs); locally the seven tiling scenarios passed as one isolation run (`agreement: pass`), while a single-process full sweep tired the SwiftShader page past scenario 24 — the shard split is the sanctioned whole-run form                                                                                                                                                                                              |

The repository gates then passed 6,944 unit tests across 174 files (re-run
2026-09-02 with the qualification's gate extensions), the four tiling sheets
(22/22: finite go/no-go, mirror-vs-alternatives, point-space decision, and
the 4D Solid representation), the production build, and the WebGL smoke.
