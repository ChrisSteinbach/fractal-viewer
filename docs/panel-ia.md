...
| **Renderer-owned Scene color**                   | Scene / Look / Renderer        | One visible section for the active renderer: Points color mode/4D color, Flame tone and palette, Solid source plus mode/gradient dependents, or Surface source/palette/speed               | Document                                                                                                                       | Use the same **Scene color** name for the same current-view question, but never show another renderer's editor beside it. Keep the Points flat/non-flat selector pair and dependents in the Points presentation. Solid mirrors the current-dimensional selector, ramp/contrast and Axis Colors inside its own presentation when **Color source** is **Color mode**; named orbit palettes override and hide those rows. The mirrors edit the one authored color definition, so Points and Solid cannot drift, but neither renderer exposes the other's section. Flame, Solid, and Surface retain their values while hidden. Surface Height/Radius may consume the stored ramp definition internally. |
