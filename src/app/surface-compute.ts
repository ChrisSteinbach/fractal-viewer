...
/**
 * The SPHERE-INVERSION kinds (docs/sphere-inversion-gpu.md): the scene-level
 * block's seed orbit on `core: "sphereInv"` / `"sphereInv4"`, named like the
 * Surface gate's routing kinds. The construction is FIXED AT CREATE (the
 * tables and header pack from `de` once; a construction edit is a session
 * restart), the ground plane composes, and `tiling` is declared absent so
 * the host's shared `target.tiling ?? null` reads stay typed. The 4D kind's
 * rotor/slice is per-frame `view4` state like every 4D kind's, and its slab
 * is refused at pack.
 *
 * Kept OUTSIDE {@link SurfaceComputeTarget} for now: that union's callers in
 * the app narrow "not forward" to the IFS kinds, which is exactly the
 * assumption these kinds break. The renderer accepts
 * {@link SurfaceComputeAnyTarget}; the app joins the two when it routes the
 * family.
 */
export type SphereInversionComputeTarget =
  | {
 ...
 * kind. */
export function isForwardTarget(target: SurfaceComputeAnyTarget): target is
  | {
      kind: "escape";
      de: EscapeDE;
      groundPlane?: boolean;
...
/** The inverse-DESCENT kinds (ifs, ifs4): the only ones whose DE carries a
 * fold final, condensation, a schedule or xaos. Named so "not forward" is
 * never read as "descent" — the sphere-inversion kinds are neither. */
export function isDescentTarget(
  target: SurfaceComputeAnyTarget,
): target is Extract<SurfaceComputeTarget, { kind: "ifs" | "ifs4" }> {
  return target.kind === "ifs" || target.kind === "ifs4";
}

/** The sphere-inversion kinds, 3D and native 4D. */
export function isSphereInversionTarget(
  target: SurfaceComputeAnyTarget,
): target is SphereInversionComputeTarget {
  return (
    target.kind === "sphereInversion" || target.kind === "sphereInversion4"
  );
}
...
/** The packed table wire and header for a session's frozen construction,
 * packed once per DE (the maps buffer and every frame's params read it). */
export function sphereInversionGpuTablesFor(
  de: SphereInversionDE,
): SphereInversionGpuTables {
  let tables = sphereInversionTables.get(de);
  if (!tables) {
    tables = packSphereInversionGpuTables(de);
    sphereInversionTables.set(de, tables);
  }
  return tables;
}
...
export function surfaceComputeTargetMeshIds(
  ...
  if (!isDescentTarget(target)) return [];
  const shapes =
...
export function isFourDTarget(target: SurfaceComputeAnyTarget): target is
  | {
      kind: "ifs4";
      de: SurfaceDE4;
      balloon?: boolean;
...
export interface SurfaceComputeRendererInit {
  ...
  /** Sphere-inversion glass only: the same continuation over the generic
...
export class SurfaceComputeRenderer {
  ...
  static async create(
    ...
    opts: {
      ...
      finiteTransportChunkPaths?: number;
      /** Sphere-inversion glass scheduling only, the finite option's twin:
       * omitted selects the production quantum, zero the uninterrupted
       * equivalence control. */
      sphereInversionTransportChunkPaths?: number;
  ...
  private static async buildOnDevice(
    ...
    const siChunkPaths =
      isSphereInversionTarget(target) && materials?.optics
        ? resolveSphereInversionTransportChunkPaths(
    ...
    const compileEntry = async (
      ...
      const module = device.createShaderModule({
        code: surfaceDeKernelWgsl({
          ...
          condensation: isDescentTarget(target)
          ...
          schedule: isDescentTarget(target)
            ? target.de.schedule &&
      ...
      entries: [
        ...
        ...(materials?.optics
          ? [
              ...
              // The sphere-inversion continuation's own slot: binding 1 is
    ...
    const mapsData = isSphereInversionTarget(target)
      ? // The sphere-inversion table wire re-types binding 1 as
        // array<vec4f> — the same buffer and layout entry every kind binds.
        new Float32Array(sphereInversionGpuTablesFor(target.de).data)
      : target.kind === "escape"
  ...
  private readonly target: SurfaceComputeAnyTarget;
  ...
  private readonly transportMaxProcessed: number;
  ...
  constructor(init: SurfaceComputeRendererInit) {
    ...
    this.transportWorkPathBytes = finiteChunk
      ? FINITE_TRANSPORT_PATH_BYTES
      : TRANSPORT_PATH_BYTES;
    // The shader's coupled path cap, per backend (the codegen's defaults):
    // the finite DDA's oracle allowance, the estimator-shaped guard else.
    this.transportMaxProcessed =
      init.transportMaxPaths ??
      (finiteChunk
        ? DIELECTRIC_MAX_PROCESSED_PATHS
        : SURFACE_GPU_TRANSPORT_MAX_PROCESSED_PATHS);
  ...
  private ensureFrameBuffers(rays: number, sampleSlots = 1): FrameBuffers {
    ...
    if (this.optics) {
      ...
      if (this.transportChunkPaths > 0) {
        transportWork = device.createBuffer({
          label:
            this.transportChunkBackend === "finiteSolid"
        ...
        stagingTransportRunning = device.createBuffer({
          label:
            this.transportChunkBackend === "finiteSolid"
              ? "finite-transport-running"
              : "si-transport-running",
          size: 4,
          usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST,
        });
      }
  ...
  private createShadeBindGroup(
    ...
    const finiteWork =
      this.transportChunkBackend === "finiteSolid" ? transportWork : undefined;
    const siWork =
  ...
  private async runFrame(
    ...
    const lensKind = isDescentTarget(this.target)
    ...
    const packParams: (run: SurfaceGpuRunParams) => ArrayBuffer =
      target.kind === "sphereInversion"
        ? (run) =>
            packSphereInversionGpuParams(
              sphereInversionGpuTablesFor(target.de),
              run,
              groundPlane,
            )
        : target.kind === "sphereInversion4"
    ...
    if (
      ...
      // judge.
      // THE FINITE continuation prices the whole multi-submission batch and
      // leaves its cap at the maximum; every other lane — the sphere-
      // inversion continuation included — is judged PER SUBMISSION against
      ...
      transportLane: while (
        ...
        for (let offset = 0; offset < pending.length;) {
          ...
          if (this.transportChunkPaths > 0 && (!work || !runningReadback))
            throw new Error(
              "Surface compute: transport continuation buffers missing",
            );
          let generation = 0;
          if (work) {
          ...
          do {
            ...
            if (work) {
              ...
              if (
                chunk >=
                Math.ceil(
                  this.transportMaxProcessed / this.transportChunkPaths,
                ) +
                  1
              )
                throw new Error(
            ...
            if (runningReadback) {
              ...
              if (counter.length !== 1 || counter[0] > previousRunning)
                throw new Error(
                  "Surface compute: invalid transport continuation running count",
                );
              running = counter[0];
              if (running > 0) transportContinuationChunks++;
            } else {
            ...
            if (running === 0) {
              ...
              transportSizer.cost = nextShadeHitCost(
                transportSizer.cost,
                slice.length,
                (chunkedTransport ? batchWorkMs : worstChunkWorkMs) * 1000,
              );
        ...
        for (let slot = 0; slot < pending.length; slot++) {
          ...
          if (this.transportChunkPaths > 0 && s === FINITE_TRANSPORT_RUNNING)
            throw new Error(
              "Surface compute: running transport trace escaped its batch",
            );
